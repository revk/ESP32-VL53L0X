...
vl53l0x_t *
vl53l0x_init (uint8_t port, uint8_t scl, uint8_t sda, uint8_t address)
{
...
VL53L0X::VL53L0X (void):address (ADDRESS_DEFAULT), io_timeout (0)
   // no timeout
   , did_timeout (false)
{
}
...
void
VL53L0X::setAddress (uint8_t new_addr)
{
...
void
VL53L0X::writeReg (uint8_t reg, uint8_t value)
{
...
void
VL53L0X::writeReg16Bit (uint8_t reg, uint16_t value)
{
   Wire.beginTransmission (address);
   Wire.write (reg);
...
void
VL53L0X::writeReg32Bit (uint8_t reg, uint32_t value)
{
   Wire.beginTransmission (address);
   Wire.write (reg);
...
void
VL53L0X::writeMulti (uint8_t reg, uint8_t const *src, uint8_t count)
{
   Wire.beginTransmission (address);
   Wire.write (reg);
...
void
VL53L0X::readMulti (uint8_t reg, uint8_t * dst, uint8_t count)
{
   Wire.beginTransmission (address);
   Wire.write (reg);
   last_status = Wire.endTransmission ();
...
float
VL53L0X::getSignalRateLimit (void)
{
...
bool VL53L0X::setVcselPulsePeriod (vcselPeriodType type, uint8_t period_pclks)
{
   ...
   if (type == VcselPeriodPreRange)
   {
      ...
      uint16_t
         new_pre_range_timeout_mclks = timeoutMicrosecondsToMclks (timeouts.pre_range_us, period_pclks);
...
void
VL53L0X::startContinuous (uint32_t period_ms)
{
   writeReg (0x80, 0x01);
   writeReg (0xFF, 0x01);
   writeReg (0x00, 0x00);
...
void
VL53L0X::getSequenceStepEnables (SequenceStepEnables * enables)
{
...
// intermediate values
void
VL53L0X::getSequenceStepTimeouts (SequenceStepEnables const *enables, SequenceStepTimeouts * timeouts)
{
